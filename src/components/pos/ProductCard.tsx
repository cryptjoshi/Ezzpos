...
export default function ProductCard({ product, onClick }: ProductCardProps & { onClick: () => void }) {
    ...
    <div className="absolute inset-0 bg-black opacity-60 rounded-2xl" /> {/* Overlay */}
      ...
      >
        <div className="flex flex-col gap-2">
          ...
          >
            <span className="text-sm opacity-80">{product.code}</span>
            <br />
            <span className="text-xs opacity-80">{product.name.slice(0, 15)}</span>
           
          </CardTitle>

          <div className="space-y-1">
            <p className="text-xs opacity-80">ราคา</p>
            <p className="text-sm font-bold">{product.price1.toFixed(2)} บาท</p>

            <p className="text-xs mt-2 opacity-80">ยอดคงเหลือ</p>
            <p className="text-xs font-medium">{product.quantity.toFixed(2)} ชิ้น</p>
          </div>
        </div>
